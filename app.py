...
"
        "- **Πνευματικά δικαιώματα:** © 2025 Παναγιώτα Γιαννίτσαρου. "
        "**Απαγορεύεται** αντιγραφή, αναδημοσίευση ή τροποποίηση χωρίς **έγγραφη άδεια**.  
"
        "- **Μη εμπορική χρήση** επιτρέπεται σε σχολεία για εσωτερική οργάνωση.  
"
        "- Παρέχεται “**ως έχει**” χωρίς εγγυήσεις. Τα αποτελέσματα έχουν **βοηθητικό** χαρακτήρα "
        "και **δεν υποκαθιστούν** κανονιστικές αποφάσεις ή παιδαγωγική κρίση.  
"
...
with st.sidebar.expander("🔒 Προστασία Δεδομένων (GDPR – Κύπρος)", expanded=False):
    st.markdown(
        "- Τα αρχεία Excel ανεβαίνουν από τον χρήστη και χρησιμοποιούνται **μόνο** για άμεσο υπολογισμό. "
        "Η εφαρμογή δεν αποθηκεύει μόνιμα δεδομένα.  
"
...
"
        "  • Αν χρησιμοποιήσετε **cloud** (π.χ. Streamlit Cloud): ελέγξτε πού φιλοξενούνται οι υποδομές, "
        "τους όρους επεξεργασίας και αποφύγετε την ανάρτηση **αναγνωρίσιμων** στοιχείων όπου δεν είναι απαραίτητο.  "
    )
...
"
        "2) **Ιδιωτικότητα & Δεδομένα:** Τα δεδομένα παρέχονται αποκλειστικά από τον χρήστη. "
        "Δεν αποθηκεύονται μόνιμα από την εφαρμογή, ούτε διαμοιράζονται σε τρίτους. "
        "Ο χρήστης παραμένει υπεύθυνος για τη συμμόρφωση με τον **GDPR** και την εθνική νομοθεσία (π.χ. ψευδωνυμοποίηση).  
"
        "3) **Περιορισμοί:** Απαγορεύεται η εμπορική εκμετάλλευση, διάθεση σε τρίτους και τροποποίηση χωρίς άδεια.  
"
        "4) **Αποποίηση Ευθύνης:** Ο/Η δημιουργός δεν ευθύνεται για αποφάσεις που λαμβάνονται αποκλειστικά με βάση τα αποτελέσματα "
        "ή για σφάλματα/ελλείψεις στα εισαγόμενα δεδομένα.  
"
